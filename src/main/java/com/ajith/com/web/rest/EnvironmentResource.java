...
/**
 * REST controller for managing {@link com.ajith.com.domain.Environment}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EnvironmentResource {
    ...
    public ResponseEntity<Environment> updateEnvironment(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, environment.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Environment> partialUpdateEnvironment(
        ...
        Optional<Environment> result = environmentRepository
            .findById(environment.getId())
            .map(existingEnvironment -> {
                if (environment.getEnvironmentName() != null) {
                    existingEnvironment.setEnvironmentName(environment.getEnvironmentName());
                }
                if (environment.getEnvironmentType() != null) {
                    existingEnvironment.setEnvironmentType(environment.getEnvironmentType());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, environment.getId().toString())
        );
    }
